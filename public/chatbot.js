...
async function startScheduleWizard(calendar) {
  ...
  if (est) {
    ...
    appendMessage('bot', `Let me craft your first month from ${sem.start.toLocaleDateString()}. Which semester are you in right now (1-8)?`);
  }
}
...
async function handleScheduleWizardAnswer(msg, calendar) {
  ...
  switch (step) {
    ...
    case 'askPriorities': {
      ...
      generateMonthlySchedule(wiz, calendar);
      return true;
    }
  }
  return false;
...
// ---------- Schedule Generation Wizard ----------
async function generateMonthlySchedule(wiz, calendar) {
  ...
  // Persist course meetings for first 4 weeks (1 month)
  for (const c of chosen) {
    await addCourseWeeks(c, null, base, 4, calendar);
  }
  ...
  const scheduleJob = async () => {
    ...
    // Hours per week - use user's exact specification
    const hoursPerWeek = wiz.data.jobHours || 10;
    
    // Use smaller shifts if user requested less than 10 hours per week
    const shiftHours = hoursPerWeek < 10 ? Math.max(2, Math.ceil(hoursPerWeek / 2)) : 5;
    const commuteMin = 50; // minutes
    
    // Calculate how many shifts we need per week
    const shiftsPerWeek = Math.ceil(hoursPerWeek / shiftHours);
    ...
    // Schedule for the first week (week 0), then copy to weeks 1, 2, 3
    const weeklySchedule = [];
    let shiftsScheduled = 0;
    
    // First, create the weekly template
    for (const [day, startHH] of candidateWindows) {
      if (shiftsScheduled >= shiftsPerWeek) break;
      
      // For the last shift, adjust hours if needed to match exact weekly total
      const currentShiftHours = (shiftsScheduled === shiftsPerWeek - 1) 
        ? hoursPerWeek - (shiftsScheduled * shiftHours)
        : shiftHours;
      
      if (currentShiftHours <= 0) continue;
      
      const slot = tryPlaceBlock(calendar, base, day, startHH, currentShiftHours, 0, 30, '20:30');
      if (!slot) continue;
      ...
      // Store the template for this shift
      weeklySchedule.push({
        day,
        startHH,
    ...
    // Now apply the weekly template to all 4 weeks
    for (let w = 0; w < 4; w++) {
      for (const shift of weeklySchedule) {
        const slot = tryPlaceBlock(calendar, base, shift.day, shift.startHH, shift.shiftHours, w, 30, '20:30');
        if (!slot) continue;
        
        const commuteStart = new Date(slot.start.getTime() - shift.commuteMin * 60000);
        const commuteEnd = new Date(slot.start);
        if (!isSlotFree(calendar, commuteStart, commuteEnd)) continue;
  ...
  const scheduleProject = async () => {
    ...
    // Hours per week - use user's exact specification
    const hoursPerWeek = wiz.data.projectHours || 6;
    
    // Use smaller blocks if user requested less than 8 hours per week
    const blockHours = hoursPerWeek < 8 ? Math.max(1, Math.ceil(hoursPerWeek / 4)) : 2;
    
    // Calculate how many blocks we need per week
    const blocksPerWeek = Math.ceil(hoursPerWeek / blockHours);
    ...
    const tryWindows = [...windows, ['Friday','16:00'], ['Monday','16:00'], ['Wednesday','14:00']];
    ...
    for (const [day, startHH] of tryWindows) {
      if (blocksScheduled >= blocksPerWeek) break;
      
      // For the last block, adjust hours if needed to match exact weekly total
      const currentBlockHours = (blocksScheduled === blocksPerWeek - 1) 
        ? hoursPerWeek - (blocksScheduled * blockHours)
        : blockHours;
      
      if (currentBlockHours <= 0) continue;
      
      const slot = tryPlaceBlock(calendar, base, day, startHH, currentBlockHours, 0, 15, '21:30');
      if (slot) {
        // Store the template for this block
        weeklySchedule.push({
          day,
          startHH,
          blockHours: currentBlockHours
        });
        blocksScheduled++;
      }
    }
    ...
    for (let w = 0; w < 4; w++) {
      for (const block of weeklySchedule) {
        const slot = tryPlaceBlock(calendar, base, block.day, block.startHH, block.blockHours, w, 15, '21:30');
        if (slot) {
          await safeAdd('Project Work', slot.start, slot.end, '#16a34a');
        }
      }
    }
  };
  ...
  // Build summary of scheduled activities
  let activitySummary = [];
  if (wiz.data.hasJob) {
    activitySummary.push(`Part-time job: ${wiz.data.jobHours || 10} hours/week`);
  }
  if (wiz.data.hasProject) {
    activitySummary.push(`Project work: ${wiz.data.projectHours || 6} hours/week`);
  }
  const activityText = activitySummary.length ? `\n${activitySummary.join(', ')}.` : '';
  
  appendMessage('bot', `${randomAck('your schedule is ready!')} I planned your first month starting ${base.toLocaleDateString()}.
Courses (${total} credits): ${courseList || 'none found for that semester'}.${activityText}
I created a weekly template and repeated it across 4 weeks, avoiding scheduling conflicts.`);
  wiz.active = false;
  wiz.step = 'idle';
}
// ---------- End Schedule Wizard ----------
