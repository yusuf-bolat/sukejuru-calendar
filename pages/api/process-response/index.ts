...
// Utility function to parse month names to numbers
...
// Utility function to generate events for specific date ranges
function generateEventsForDateRange(title: string, dayName: string, startTime: string, endTime: string, startDate: string, endDate: string): any[] {
  const events = []
  const daysOfWeek = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday']
  const targetDayIndex = daysOfWeek.indexOf(dayName.toLowerCase())
  ...
  try {
    // Handle different date formats like "August 30th" or "2024-08-30"
    if (startDate.includes(' ')) {
      // Handle "August 30th" format
      const startParts = startDate.replace(/(\d+)(st|nd|rd|th)/g, '$1').split(' ')
      if (startParts.length >= 2) {
        const month = parseMonthName(startParts[0])
    ...
    if (endDate.includes(' ')) {
      // Handle "September 21st" format
      const endParts = endDate.replace(/(\d+)(st|nd|rd|th)/g, '$1').split(' ')
      if (endParts.length >= 2) {
        const month = parseMonthName(endParts[0])
    ...
    console.log(`Generating events from ${rangeStart.toDateString()} to ${rangeEnd.toDateString()} for ${dayName}s`)
    
  } catch (error) {
    console.error('Error parsing date range:', error)
    // Fallback to recurring events
    return generateRecurringEvents(title, dayName, startTime, endTime, 2)
  }
  
  // Find the first occurrence of the target day within or after the start date
  ...
  while (currentDate <= rangeEnd) {
    // Parse time strings (e.g., "15:00", "17:00" for 3pm-5pm)
    ...
    events.push({
      ...
      all_day: false,
      color: '#3788d8',
      description: `${dayName} job: ${startTime}-${endTime}`
    })
    
    // Move to next week (next occurrence of the same day)
    currentDate.setDate(currentDate.getDate() + 7)
  }
  
  console.log(`Generated ${events.length} events for ${dayName}s from ${rangeStart.toDateString()} to ${rangeEnd.toDateString()}`)
  return events
}
